...
function $14e64092bb8e46d2258aa0c58e5c08ac$export$default(ids, coords, nodeSize, left, right, depth) {
  ...
  $14e64092bb8e46d2258aa0c58e5c08ac$var$select(ids, coords, m, left, right, depth % 2);
  $14e64092bb8e46d2258aa0c58e5c08ac$export$default(ids, coords, nodeSize, left, m - 1, depth + 1);
  $14e64092bb8e46d2258aa0c58e5c08ac$export$default(ids, coords, nodeSize, m + 1, right, depth + 1);
}
function $14e64092bb8e46d2258aa0c58e5c08ac$var$select(ids, coords, k, left, right, inc) {
  while (right > left) {
    if (right - left > 600) {
      ...
      $14e64092bb8e46d2258aa0c58e5c08ac$var$select(ids, coords, k, newLeft, newRight, inc);
    }
    const t = coords[2 * k + inc];
    let i = left;
    let j = right;
    $14e64092bb8e46d2258aa0c58e5c08ac$var$swapItem(ids, coords, left, k);
    if (coords[2 * right + inc] > t) $14e64092bb8e46d2258aa0c58e5c08ac$var$swapItem(ids, coords, left, right);
    while (i < j) {
      $14e64092bb8e46d2258aa0c58e5c08ac$var$swapItem(ids, coords, i, j);
      i++;
      j--;
      while (coords[2 * i + inc] < t) i++;
      while (coords[2 * j + inc] > t) j--;
    }
    if (coords[2 * left + inc] === t) $14e64092bb8e46d2258aa0c58e5c08ac$var$swapItem(ids, coords, left, j); else {
      j++;
      $14e64092bb8e46d2258aa0c58e5c08ac$var$swapItem(ids, coords, j, right);
    }
    if (j <= k) left = j + 1;
    if (k <= j) right = j - 1;
  }
}
function $14e64092bb8e46d2258aa0c58e5c08ac$var$swapItem(ids, coords, i, j) {
  $14e64092bb8e46d2258aa0c58e5c08ac$var$swap(ids, i, j);
  $14e64092bb8e46d2258aa0c58e5c08ac$var$swap(coords, 2 * i, 2 * j);
  $14e64092bb8e46d2258aa0c58e5c08ac$var$swap(coords, 2 * i + 1, 2 * j + 1);
}
function $14e64092bb8e46d2258aa0c58e5c08ac$var$swap(arr, i, j) {
  const tmp = arr[i];
  arr[i] = arr[j];
  arr[j] = tmp;
}
function $eef7c2716e94c10b0d2204becb4b55a1$export$default(ids, coords, minX, minY, maxX, maxY, nodeSize) {
...
function $1033f4c4dea9b488065b6eddb394f033$export$default(ids, coords, qx, qy, r, nodeSize) {
  const stack = [0, ids.length - 1, 0];
  const result = [];
  const r2 = r * r;
  while (stack.length) {
    ...
    if (right - left <= nodeSize) {
      for (let i = left; i <= right; i++) {
        if ($1033f4c4dea9b488065b6eddb394f033$var$sqDist(coords[2 * i], coords[2 * i + 1], qx, qy) <= r2) result.push(ids[i]);
      }
      continue;
    }
    const m = Math.floor((left + right) / 2);
    ...
    if ($1033f4c4dea9b488065b6eddb394f033$var$sqDist(x, y, qx, qy) <= r2) result.push(ids[m]);
    const nextAxis = (axis + 1) % 2;
...
function $1033f4c4dea9b488065b6eddb394f033$var$sqDist(ax, ay, bx, by) {
  const dx = ax - bx;
  const dy = ay - by;
  return dx * dx + dy * dy;
}
const $ec3a0e7135bf1313da05c68a69420c29$var$defaultGetX = p => p[0];
const $ec3a0e7135bf1313da05c68a69420c29$var$defaultGetY = p => p[1];
class $ec3a0e7135bf1313da05c68a69420c29$export$default {
  constructor(points, getX = $ec3a0e7135bf1313da05c68a69420c29$var$defaultGetX, getY = $ec3a0e7135bf1313da05c68a69420c29$var$defaultGetY, nodeSize = 64, ArrayType = Float64Array) {
    ...
    $14e64092bb8e46d2258aa0c58e5c08ac$export$default(ids, coords, nodeSize, 0, ids.length - 1, 0);
  }
  range(minX, minY, maxX, maxY) {
    return $eef7c2716e94c10b0d2204becb4b55a1$export$default(this.ids, this.coords, minX, minY, maxX, maxY, this.nodeSize);
  }
  within(x, y, r) {
    return $1033f4c4dea9b488065b6eddb394f033$export$default(this.ids, this.coords, x, y, r, this.nodeSize);
  }
}
const $cf9bb3333627547726792c50f5fb3c08$var$defaultOptions = {
  minZoom: 0,
...
const $cf9bb3333627547726792c50f5fb3c08$var$fround = Math.fround || (tmp => x => {
  tmp[0] = +x;
  return tmp[0];
})(new Float32Array(1));
class $cf9bb3333627547726792c50f5fb3c08$export$default {
  constructor(options) {
    this.options = $cf9bb3333627547726792c50f5fb3c08$var$extend(Object.create($cf9bb3333627547726792c50f5fb3c08$var$defaultOptions), options);
  ...
  load(points) {
    ...
    for (let i = 0; i < points.length; i++) {
      ...
      clusters.push($cf9bb3333627547726792c50f5fb3c08$var$createPointCluster(points[i], i));
    }
    this.trees[maxZoom + 1] = new $ec3a0e7135bf1313da05c68a69420c29$export$default(clusters, $cf9bb3333627547726792c50f5fb3c08$var$getX, $cf9bb3333627547726792c50f5fb3c08$var$getY, nodeSize, Float32Array);
    ...
    for (let z = maxZoom; z >= minZoom; z--) {
      ...
      this.trees[z] = new $ec3a0e7135bf1313da05c68a69420c29$export$default(clusters, $cf9bb3333627547726792c50f5fb3c08$var$getX, $cf9bb3333627547726792c50f5fb3c08$var$getY, nodeSize, Float32Array);
  ...
  getClusters(bbox, zoom) {
    ...
    const ids = tree.range($cf9bb3333627547726792c50f5fb3c08$var$lngX(minLng), $cf9bb3333627547726792c50f5fb3c08$var$latY(maxLat), $cf9bb3333627547726792c50f5fb3c08$var$lngX(maxLng), $cf9bb3333627547726792c50f5fb3c08$var$latY(minLat));
    const clusters = [];
    for (const id of ids) {
      const c = tree.points[id];
      clusters.push(c.numPoints ? $cf9bb3333627547726792c50f5fb3c08$var$getClusterJSON(c) : this.points[c.index]);
    }
    return clusters;
  }
  getChildren(clusterId) {
    ...
    for (const id of ids) {
      ...
      if (c.parentId === clusterId) {
        children.push(c.numPoints ? $cf9bb3333627547726792c50f5fb3c08$var$getClusterJSON(c) : this.points[c.index]);
      }
    }
  ...
  _addTileFeatures(ids, points, x, y, z2, tile) {
    for (const i of ids) {
      ...
      let tags, px, py;
      if (isCluster) {
        tags = $cf9bb3333627547726792c50f5fb3c08$var$getClusterProperties(c);
        px = c.x;
        py = c.y;
      } else {
        const p = this.points[c.index];
        tags = p.properties;
        px = $cf9bb3333627547726792c50f5fb3c08$var$lngX(p.geometry.coordinates[0]);
        py = $cf9bb3333627547726792c50f5fb3c08$var$latY(p.geometry.coordinates[1]);
      }
      const f = {
        type: 1,
  ...
  _cluster(points, zoom) {
    ...
    for (let i = 0; i < points.length; i++) {
      ...
      if (numPoints >= minPoints) {
        ...
        p.parentId = id;
        clusters.push($cf9bb3333627547726792c50f5fb3c08$var$createCluster(wx / numPoints, wy / numPoints, id, numPoints, clusterProperties));
      } else {
  ...
  _map(point, clone) {
    if (point.numPoints) {
      return clone ? $cf9bb3333627547726792c50f5fb3c08$var$extend({}, point.properties) : point.properties;
    }
    const original = this.points[point.index].properties;
    const result = this.options.map(original);
    return clone && result === original ? $cf9bb3333627547726792c50f5fb3c08$var$extend({}, result) : result;
  }
}
function $cf9bb3333627547726792c50f5fb3c08$var$createCluster(x, y, id, numPoints, properties) {
  return {
    x: $cf9bb3333627547726792c50f5fb3c08$var$fround(x),
    // weighted cluster center; round for consistency with Float32Array index
    y: $cf9bb3333627547726792c50f5fb3c08$var$fround(y),
    zoom: Infinity,
...
function $cf9bb3333627547726792c50f5fb3c08$var$createPointCluster(p, id) {
  const [x, y] = p.geometry.coordinates;
  return {
    x: $cf9bb3333627547726792c50f5fb3c08$var$fround($cf9bb3333627547726792c50f5fb3c08$var$lngX(x)),
    // projected point coordinates
    y: $cf9bb3333627547726792c50f5fb3c08$var$fround($cf9bb3333627547726792c50f5fb3c08$var$latY(y)),
    zoom: Infinity,
...
function $cf9bb3333627547726792c50f5fb3c08$var$getClusterJSON(cluster) {
  return {
    type: 'Feature',
    id: cluster.id,
    properties: $cf9bb3333627547726792c50f5fb3c08$var$getClusterProperties(cluster),
    geometry: {
      type: 'Point',
      coordinates: [$cf9bb3333627547726792c50f5fb3c08$var$xLng(cluster.x), $cf9bb3333627547726792c50f5fb3c08$var$yLat(cluster.y)]
    }
  };
}
function $cf9bb3333627547726792c50f5fb3c08$var$getClusterProperties(cluster) {
  const count = cluster.numPoints;
  const abbrev = count >= 10000 ? `${Math.round(count / 1000)}k` : count >= 1000 ? `${Math.round(count / 100) / 10}k` : count;
  return $cf9bb3333627547726792c50f5fb3c08$var$extend($cf9bb3333627547726792c50f5fb3c08$var$extend({}, cluster.properties), {
    cluster: true,
...
function $cf9bb3333627547726792c50f5fb3c08$var$lngX(lng) {
  return lng / 360 + 0.5;
}
function $cf9bb3333627547726792c50f5fb3c08$var$latY(lat) {
  const sin = Math.sin(lat * Math.PI / 180);
  const y = 0.5 - 0.25 * Math.log((1 + sin) / (1 - sin)) / Math.PI;
  return y < 0 ? 0 : y > 1 ? 1 : y;
}
// spherical mercator to longitude/latitude
function $cf9bb3333627547726792c50f5fb3c08$var$xLng(x) {
  return (x - 0.5) * 360;
}
function $cf9bb3333627547726792c50f5fb3c08$var$yLat(y) {
  const y2 = (180 - y * 360) * Math.PI / 180;
  return 360 * Math.atan(Math.exp(y2)) / Math.PI - 90;
}
function $cf9bb3333627547726792c50f5fb3c08$var$extend(dest, src) {
  for (const id in src) dest[id] = src[id];
  return dest;
}
function $cf9bb3333627547726792c50f5fb3c08$var$getX(p) {
  return p.x;
}
function $cf9bb3333627547726792c50f5fb3c08$var$getY(p) {
  return p.y;
}
/**
* @module helpers
...
class $a766a508761edee757e2f1e9b8864eca$export$default {
  ...
  */
  constructor(schema) {
    this.schema = this.index = new $cf9bb3333627547726792c50f5fb3c08$export$default();
  ...
  */
  indexData(schemaHelper) {
    ...
    // data loop.
    if (schemaHelper.xScale instanceof $3ebbd6da2f6dbbd2ea0ba22bdc176caf$init().GenomeScale) {
    ...
    if (schemaHelper.yScale instanceof $3ebbd6da2f6dbbd2ea0ba22bdc176caf$init().GenomeScale) {
