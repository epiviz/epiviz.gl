define(['exports', './utilities-bf9f0c79'], function (exports, utilities) { 'use strict';
  ...
  function cubehelixConvert(o) {
    ...
    if (!(o instanceof utilities.Rgb)) o = utilities.rgbConvert(o);
    var r = o.r / 255,
        g = o.g / 255,
        b = o.b / 255,
  ...
  utilities.define(Cubehelix, cubehelix$2, utilities.extend(utilities.Color, {
    brighter: function(k) {
      k = k == null ? utilities.brighter : Math.pow(utilities.brighter, k);
      return new Cubehelix(this.h, this.s, this.l * k, this.opacity);
    },
    darker: function(k) {
      k = k == null ? utilities.darker : Math.pow(utilities.darker, k);
      return new Cubehelix(this.h, this.s, this.l * k, this.opacity);
    },
    rgb: function() {
  ...
  const getCurveForArc = (P0, P2) => {
    ...
    if (!isFinite(slope)) {
      // vertical slope
      return utilities.getQuadraticBezierCurveForPoints(
        P0,
  ...
  class VertexCalculator {
    ...
    constructor(xScale, yScale, track) {
      if (xScale instanceof utilities.GenomeScale) {
        this.xScale = xScale.toCallable();
      } else {
        this.xScale = xScale;
      }

      if (yScale instanceof utilities.GenomeScale) {
  ...
  class SchemaProcessor {
    ...
    constructor(schema, callback) {
      ...
      if (Array.isArray(schema.defaultData)) {
        ...
        this.dataPromise = undefined(schema.defaultData).then((response) => {
      ...
      this.xScale = utilities.getScaleForSchema("x", schema);
      this.yScale = utilities.getScaleForSchema("y", schema);
  ...
  class Track {
    ...
    constructor(schema, track) {
      ...
      if (Array.isArray(track.data)) {
        ...
        this.dataPromise = undefined(track.data).then((response) => {
          this.data = response.data.split(/[\n\r]+/);
          this.processHeadersAndMappers();
          this.hasOwnData = true;
        });
    ...
    buildMapperForChannel = (channel) => {
      if (channel in this.track) {
        ...
        if ("value" in channelInfo) {
          if (channel === "color") {
            channelInfo.value = utilities.colorSpecifierToHex(channelInfo.value);
          }
  ...
  const buildMapperForQuantitiveChannel = (channel, channelInfo) => {
    switch (channel) {
      ...
      case "opacity":
        return utilities.scale(channelInfo.domain, [
      ...
      case "color":
        ...
        const zeroToOneScale = utilities.scale(channelInfo.domain, [0, 1]);
        return (attrValue) =>
          utilities.rgbStringToHex(d3colorScale(zeroToOneScale(attrValue)));
      case "width":
        return utilities.scale(channelInfo.domain, [
  ...
  const buildMapperForCategoricalChannel = (channel, channelInfo) => {
    ...
    switch (channel) {
      case "x":
      case "y":
        channelScale = utilities.scale([0, channelInfo.cardinality], [-1, 1]);
        break;
      case "opacity":
        channelScale = utilities.scale(
          [
            channelInfo.minOpacity || DEFAULT_MIN_OPACITY,
      ...
      case "color":
        ...
        const zeroToOneScale = utilities.scale([0, channelInfo.cardinality], [0, 1]);
        channelScale = (categoryId) =>
          utilities.rgbStringToHex(d3colorScale(zeroToOneScale(categoryId)));
        break;
      case "width":
        channelScale = utilities.scale(
          [0, channelInfo.cardinality],
  ...
  exports.VertexCalculator = VertexCalculator;
  exports.getDrawModeForTrack = getDrawModeForTrack;
